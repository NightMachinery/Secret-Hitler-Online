...
start_postgres_session () {
  tmuxnew "$POSTGRES_SESSION" "env PG_BIN_DIR=${(qqq)PG_BIN_DIR} POSTGRES_STATE_DIR=${(qqq)POSTGRES_STATE_DIR} POSTGRES_DATA_DIR=${(qqq)POSTGRES_DATA_DIR} POSTGRES_SOCKET_DIR=${(qqq)POSTGRES_SOCKET_DIR} POSTGRES_PASSWORD_FILE=${(qqq)POSTGRES_PASSWORD_FILE} POSTGRES_LOG_FILE=${(qqq)POSTGRES_LOG_FILE} POSTGRES_PORT=${(qqq)POSTGRES_PORT} POSTGRES_DB=${(qqq)POSTGRES_DB} POSTGRES_USER=${(qqq)POSTGRES_USER} bash ${(qqq)POSTGRES_BOOT_SCRIPT}"
}

start_backend_session () {
  local gradle_task="${1:-run}"
  tmuxnew "$BACKEND_SESSION" "env BACKEND_DIR=${(qqq)BACKEND_DIR} PG_BIN_DIR=${(qqq)PG_BIN_DIR} DATABASE_URL_LOCAL=${(qqq)DATABASE_URL_LOCAL} POSTGRES_PORT=${(qqq)POSTGRES_PORT} POSTGRES_USER=${(qqq)POSTGRES_USER} BACKEND_PORT=${(qqq)BACKEND_PORT} BACKEND_GRADLE_TASK=${(qqq)gradle_task} BACKEND_GRADLE_ARGS=${(qqq)BACKEND_GRADLE_ARGS} BACKEND_GRADLE_PROXY_OPTS=${(qqq)BACKEND_GRADLE_PROXY_OPTS} PUBLIC_ORIGIN=${(qqq)PUBLIC_ORIGIN} bash ${(qqq)BACKEND_BOOT_SCRIPT}"
}
...
start_frontend_session () {
  ...
  tmuxnew "$FRONTEND_SESSION" "env FRONTEND_DIR=${(qqq)FRONTEND_DIR} NODE_VERSION=${(qqq)NODE_VERSION} FRONTEND_PORT=${(qqq)FRONTEND_PORT} PUBLIC_ORIGIN=${(qqq)PUBLIC_ORIGIN} UNLOCK_ALL_P=${(qqq)UNLOCK_ALL_P} zsh ${(qqq)frontend_boot_script}"
}
